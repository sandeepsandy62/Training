/*
 approach 1 - greedy: 
 points to be noted : 
 non-negative , non - decreasing order - including zero and elements need not be distinct
 to make the median maximum we can make the first half before median to zero
 and make the other half after the median  , ele_after_median : len / 2 + 1 elements
 we can divide the sum / ele_after_median , if we have any remainder add it to the last elements
 such that sum of the elements == sum condition satisfied
 by adding remainder to the last element makes the array non decreasing 
 and this can be maximum possible median for the given problem
 
 code : 
            int medianIndex = n/2 ;
            int medianValue = s / (medianIndex+1);

            sb.append(medianValue).append("\n");
*/
